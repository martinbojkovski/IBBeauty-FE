...
const Reservations = ({ token }) => {
    ...
    const fetchReservations = async () => {
        try {
            const response = await fetch("https://64.227.123.235:8080/reservation", {
            });
    ...
    const handleDeleteReservation = async () => {
        try {
            const response = await fetch(`https://64.227.123.235:8080/reservation/delete/${selectedEvent.id}`, {
                method: "DELETE",
